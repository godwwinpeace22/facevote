                    ...
                    <v-layout row="">
                      <v-flex xs12 d-flex>
                        <v-text-field required small v-model="form.name" style="text-transform:capitalize"
                         color="pink" browser-autocomplete="name" :placeholder="$store.state.user.name" 
                         label="Your Name">
                         </v-text-field>
                      </v-flex>
                    </v-layout>
              ...
              <v-flex xs10 offset-sm-3>
                <v-card tile>
                  ...
                  <v-container v-if="form.is_student">
                    <v-layout column>
                      <v-flex xs10 d-flex>
                        <v-select required small :disabled="disabled_fields" v-model="form.school" :items="schools" color="pink" label="School" ></v-select>
                      </v-flex>
                      <v-flex xs8 d-flex>
                        <v-select required small :disabled="disabled_fields" v-model="form.faculty" :items="faculties[form.school]" color="pink" label="Select faculty" ></v-select>
                      </v-flex>
                      <v-flex xs8 d-flex>
                        <v-select required small :disabled="disabled_fields" v-model="form.department" :items="departments[form.school]" color="pink" label="Select department"></v-select>
                      </v-flex>
                    </v-layout>
                  </v-container>
                </v-card>
                ...
                <v-card tile height="380">
                  <v-hover v-if="!selected_file">
                    <v-container slot-scope="{ hover }">
                      <v-img :src=" $store.getters.getUser.imgSrc || `https://ui-avatars.com/api/?name=${$store.getters.getUser.name}`" max-height="250" @click="openFileSelect">
                        <v-expand-transition>
...
export default {
  ...
  methods:{
    ...
    async updatePhoto(){
      ...
      formData.append('public_id', this.$store.getters.getUser.username + Date.now())
      try {
        ...
        this.$store.dispatch('setUser', {user:res2.data,token:this.$store.getters.getToken})
    ...
    async save(){
      ...
      try {
        ...
        this.$store.dispatch('setUser', {user:saved.data,token:this.$store.getters.getToken})
    ...
    setCurrSettings(data){
      ...
      if(this.me){
        ...
        this.form.school = this.me.school
        this.form.department = this.me.department
        this.form.faculty = this.me.faculty
  ...
  mounted(){
    this.setCurrSettings(this.$store.state.logged_in_user)
  },
  components:{
  }
}

  ...
  methods:{
    async initialize () {
      ...
      this.$gUser.get('subs').on(async sub => {
        ...
        if(Date.now() - thirty_days > dec.date_created){
    ...
    async aliasIsTaken(alias){
      ...
      let u = await this.$gun.get('users').get(alias).then()

      return !!u

    },
    async checkUserIn(userData){
      
      let username = userData.username.split('.')[0]
      let isTaken = await this.aliasIsTaken(username)

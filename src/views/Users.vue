<template>
  <div>
    ...
    <template v-else>
      <!-- ==STATS CARDS== -->
      <!-- <v-container grid-list-xl fluid class="" v-if="$vuetify.breakpoint.smAndUp">
        <v-layout row wrap justify-center align-center>
          <v-flex xs6 sm3 v-for="stat in stats" :key="stat.title">
            <v-card class="round elevation-1 text-xs-center" :height="$vuetify.breakpoint.xsOnly ? 50 : 120">
      ...
      <v-container :grid-list-md="$vuetify.breakpoint.smAndUp" :pa-0="$vuetify.breakpoint.xsOnly">
        <v-layout row wrap>
          ...
          <!-- PROFILE -->
          <v-flex xs12 sm3>
            <v-card :class="{round_top: $vuetify.breakpoint.smAndUp}" height="465">
              ...
              <v-sheet flat width="61%" height="120" style="position: absolute;top: 55px;left: 20%;" color="transparent">
                ...
                >
                  ...
                  <span v-else class="white--text display-2">{{user.name.charAt(0)}}</span>
                </v-avatar>
              </v-sheet>
              ...
              <v-card-text class="text-xs-center " style="margin-top: 65px;">
                ...
                <div class="" v-if="user.is_student">Student at {{user.sch}},</div>
                <div class="" v-if="user.is_student">Department of {{user.dept}}</div>
              </v-card-text>
              <v-card-actions class="justify-center" >
                <template v-if="user.uid != getUser.uid">
                  ...
                  <v-btn color="secondary" class="round" @click="follow">
          ...
          <v-flex xs12 sm9>
            <v-card :class="{'round_top': $vuetify.breakpoint.smAndUp}" :flat="$vuetify.breakpoint.xsOnly" style="min-height: 400px;">
              <v-toolbar dense flat card tabs color="grey lighten-3">
                ...
                >
                  <v-tab 
                    v-for="item in ['overview', 'posts', 'broadcasts']"
                    :key="item"
                    :href="`#${item}`"
                  >
                    {{ item }}
                ...
                <v-tabs-items v-model="model">
                  ...
                  <v-tab-item value="overview" >
                    <v-card flat style="border: 1px solid #eee;">
                      <v-card-text >
                        ...
                        <v-layout row wrap justify-center align-center>
                          <v-flex xs6 sm3 v-for="stat in stats" :key="stat.title">
                            <v-card class="text-xs-center" flat style="border: 1px solid #eee;" :height="$vuetify.breakpoint.xsOnly ? 100 : 140">
                  ...
                  <v-tab-item value="posts" >
                    <v-card flat>
                      ...
                      <v-layout row wrap class="navdrawr" style="height:350px;overflow:auto;">
                  ...
                  <v-tab-item value="broadcasts" >
                    <v-card flat>
                      <v-card-text>Broadcasts</v-card-text>
                    </v-card>
                  </v-tab-item>
          ...
          <v-flex xs12 sm4 d-flex>
            <v-card height="400" class="round_top">
              <v-tabs color="grey lighten-3" class="round_top" show-arrows slider-color="secondary">
                ...
                <v-tab href="#tab-1" class="text-capitalize"> Managing </v-tab>
                    ...
                    <v-list two-line>
                      <template v-for="(election, i) in myCreated">
                        <v-list-tile avatar :key="election.electionId  + i" @click="manage_dialog = true; managing = election.electionId">
                          ...
                          <!-- <v-list-tile-action>
                            <v-menu offset-y v-if="getUser.email == $route.params.email">
                              <v-btn color="success" v-if="getUser.email == $route.params.email" depressed class="text-capitalize" dark slot="activator">
                                Manage
                                <v-icon>arrow_drop_down</v-icon>
                              </v-btn>
                              <v-list dense>
                                <v-list-tile :to="`/elections/watch/${election.electionId}`">
                                  <v-list-tile-title>Election Page</v-list-tile-title>
                                </v-list-tile>
                                <v-list-tile @click="manage_dialog = true; managing = election.electionId">
                ...
                <v-tab-item value="tab-2">
                  ...
                  <v-list two-line>
                    <template v-for="(election, i) in myContests" >
                      <v-list-tile avatar :key="election.dateCreated  + i" @click="''">
                        <v-list-tile-content>
                          <v-list-tile-title>{{election.title}}</v-list-tile-title>
                          <v-list-tile-sub-title>
                            <span class="online_badge" :class="[electionStatus(election)[1]]"></span>
                ...
                <v-tab-item value="tab-3">
                  ...
                  <v-list two-line>
                    <template v-for="(election, i) in myEnrolled">
                      <v-list-tile avatar @click="''" :key="election.electionId  + election.dateCreated">
                        <v-list-tile-content>
                          <v-list-tile-title>{{election.title}}</v-list-tile-title>
                          <v-list-tile-sub-title>
                      ...
                      <v-divider :key="election.electionId  + i + election.dateCreated"></v-divider>
                    </template>
                  </v-list>
                </v-tab-item>
              </v-tabs>
            ...
            <v-card class="round_top" height="400" >
              ...
              <v-list dense two-line v-for="follower in followers" :key="follower.uid">
                <v-list-tile avatar @click="$eventBus.$emit('ViewProfile', follower.onr)">
                  ...
                  <v-list-tile-content>
                    <v-list-tile-title >{{follower.onr.name}}</v-list-tile-title>
          ...
          <v-flex xs12 sm4 d-flex>
            <v-card class="round_top">
              <v-toolbar dense flat>
                ...
                <v-btn color="secondary" icon small v-if="canCreateManifesto" class="text-capitalize" @click="$eventBus.$emit('OpenNewManifestoDialog')">
                  <v-icon>add</v-icon>
                </v-btn>

                <v-btn color="secondary" icon small v-if="getUser.email == $route.params.email && !isSuperUser" class="text-capitalize" @click="$eventBus.$emit('Show_Upgrade_Dialog', {from:'new_manifesto'})">
                  <v-icon>add</v-icon>
                </v-btn>
              </v-toolbar>
              <v-card-text class="pa-0 navdrawr" style="max-height:310px;overflow:auto;">
                <v-list two-line>
                  ...
                  <template v-for="(manifesto,i) in manifestos" >
                    <v-list-tile avatar :key="i" @click="view_manifesto = true; viewManifesto(manifesto)">
                      <v-list-tile-content>
                        ...
                        <v-list-tile-sub-title>
                          ...
                          <v-icon small>remove_red_eye</v-icon> {{manifesto.seenBy ? manifesto.seenBy.length : 0}} |
                          {{manifesto.manifesto_type}}
                        </v-list-tile-sub-title>
                      ...
                      <v-list-tile-action>
                        ...
                        <v-dialog v-model="deleteDialog" persistent max-width="290">
                          ...
                          <v-card>
                            <v-card-title class="headline">Delete Manifesto</v-card-title>
                            <v-card-text>Delete this manifesto</v-card-text>
                            <v-card-actions>
                              <v-spacer></v-spacer>
      ...
      <v-dialog v-model="manage_dialog" fullscreen lazy
        ...
        <v-card tile>
          <v-toolbar card color="secondary" flat dark>
            ...
            <v-spacer></v-spacer>
            <v-btn color="" icon @click="manage_dialog = false">
              <v-icon>close</v-icon>
            </v-btn>
          </v-toolbar>

          <v-card-text class="pa-0">
            <election-manager :electionid='managing'/>
          </v-card-text>
        </v-card>
      </v-dialog>
        ...
        <v-card :style="viewPostStyle">
          ...
          <v-card-text>
            ...
            <v-container grid-list-sm class="px-1" v-if="cur_post.imgs.length > 0">
              <v-layout row wrap>
                <v-flex v-for="(image,i) in cur_post.imgs" :key="i" class="px-2" 
                  :class="{'xs3': cur_post.imgs.length >=3,'xs12': cur_post.imgs.length == 1,'xs6': cur_post.imgs.length == 2}">
                  <v-card dark max-height="500px" :height="cur_post.imgs.length == 1 ? '100%' : $vuetify.breakpoint.xsOnly ? 100 : '200'" class="mb-3">
          ...
          <v-card-actions  :style="viewPostAction" class="grey lighten-3">
            <v-btn color="blue darken-1" flat @click="likePost(cur_post)">
      ...
      <v-dialog v-model="view_manifesto" content-class="white view_dialog"
        ...
        <v-card>
          <v-list two-line dark class="orange">
            <v-list-tile avatar>
              <v-list-tile-action class="hidden-sm-and-up" style="min-width: 30px;">
          ...
          <v-card-text>
            <img v-if="viewing.manifesto_type == 'photo'" :src="viewing.imgSrc" alt="manifesto_img" srcset="">
            <div>{{viewing.img_caption}}</div>
            <div v-if="viewing.manifesto_type == 'text'">
              <div v-html="viewing.manifesto_text"></div>
            </div>
            <div v-if="viewing.manifesto_type == 'video'">
              <iframe allowfullscreen frameborder="0" width="100%" height="345" src="https://www.youtube.com/embed/tgbNymZ7vqY?controls=1">
              </iframe>
            </div>
          </v-card-text>

          <v-card-actions>
            <v-btn icon @click="likeManifesto(viewing)">
              <v-icon color="red">favorite</v-icon> {{viewing.likes ? viewing.likes.length : '0'}}
            </v-btn>
            <v-btn icon>
              <v-icon >remove_red_eye</v-icon> 
              {{viewing.seenBy ? viewing.seenBy.length : 0}}
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
export default {
  ...
  computed:{
    ...
    stats(){
      return [
        {title:'Followers', value: '500', text: '65 new this week', icon: 'lens',iconColor:'secondary'},
        {title:'Contests', value: '644', text: 'Since last month', icon:'group', iconColor:'purple'},
        {title:'Elections', value: 44, text: '5 roles/positions', icon: 'hdr_strong', iconColor:'success'},
        {title:'Posts', value: '103', text: 'Since last month', icon: 'equalizer', iconColor: 'cyan'}
      ]
    },
    lowercase(val){
  ...
  methods:{
    ...
    deleteManifesto(manifesto){
      ...
      .doc(this.curRoom.electionId + '-' + this.getUser.uid)
    ...
    async getContested(){
      ...
      this.myContests = elecs.sort((a,b) => b.dateCreated - a.dateCreated)
    },
    async getEnrolled(){
      ...
      this.myEnrolled = arr.sort((a,b) => b.dateCreated - a.dateCreated)
    },
    ...
    likePost(post){
      ...
      let index = likes.indexOf(this.getUser.uid)
      let docRef = db.collection('posts').doc(post.id)
      if(index != -1){
        this.post_likes = post.likes.splice(index,1)
        docRef.update({
          likes: firebase.firestore.FieldValue.arrayRemove(this.getUser.uid)
        })
      }
      else{
        console.log('adding like',index,this.post_likes)
        this.post_likes = [...likes,this.getUser.uid]
        docRef.update({
          likes: firebase.firestore.FieldValue.arrayUnion(this.getUser.uid)
        })
      }
    ...
    async getManifestos(limit){
      this.manifestoRef = db.collection('manifestos').where('by','==',this.user.uid)
